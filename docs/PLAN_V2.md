...
| 0.5 — CI and reproducible verification | **DONE for the JVM core; branch protection outstanding** | `verifyAll` is the local aggregate gate. `.github/workflows/ci.yml` runs `./gradlew verifyAll` on JDK 21, validates the Gradle wrapper checksum, uploads test reports, and runs `scripts/firebase-guard.sh`, `scripts/float-guard.sh`, a tracked-binary guard, a 2 MiB file-size guard, and secret scanning. Branch protection is a repository setting and cannot be evidenced from source; it must be enabled on `main` before merge. An Android job is deliberately absent until `:android:app` exists, so no gate reports green for work that does not exist. The repository's Actions policy permits GitHub-owned actions only and requires commit-SHA pinning, so wrapper validation and secret scanning are implemented as scripts in `scripts/` rather than third-party actions; a side benefit is that both run identically on a developer machine. Run 32533380168 on `rebuild/phase-0-3-kotlin-core` completed with all three jobs green, and both guards were verified to fail when a violation is introduced. |
...
| 4 — event/schema evolution and migrations | **DONE** | `EventRegistry.RELEASED` declares all eight `EventType` constants with owner, purpose, compatibility policy and per-version payload fields (`EventSchema.kt`, `EventRegistry.kt`); `RegisteredEventStore` validates on append and upcasts on read, dead-lettering what it cannot lift. `SqliteMigrator` keeps a checksummed `schema_migrations` ledger and refuses an edited migration, a newer database, or a gapped ledger. `SchemaFixtureCompatibilityTest` replays the retained text fixture `core/persistence/src/test/resources/fixtures/schema-v1/events.sql` and asserts the version 1 rows still decode, are not rewritten, and survive a forward upgrade. `docs/events/EVENT_REGISTRY.md` is generated and verified by `EventRegistryDocsTest`; authoring rules are in `docs/persistence/MIGRATIONS.md` (ADR-0014, ADR-0015). |
...
| 4 — Event/schema evolution and migrations **DONE** | Define compatibility before additional persisted concepts are introduced. | Event registry with owner and compatibility policy (`EventRegistry.RELEASED`), read-path upcasts, registry-enforcing store decorator, generated registry document, checksummed migration ledger, retained text schema fixtures, authoring guide (`docs/persistence/MIGRATIONS.md`). | Existing: `./gradlew :core:persistence:test --tests '*Migration*' --tests '*Fixture*'` (also a named CI step); `./gradlew :core:eventbus:test`; `SchemaFixtureCompatibilityTest` decodes the retained version 1 fixture; `SqliteMigratorTest` rejects a mutated migration checksum, a gapped ledger and a newer database; `EventRegistryDocsTest` fails when `docs/events/EVENT_REGISTRY.md` drifts from the declaration. Types whose producers arrive in later phases remain `Draft` and are ratified by those phases. |
